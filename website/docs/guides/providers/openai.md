...
For DeepSeek, Qwen/Bailian, Kimi, and GLM, use the shared
`OpenAICompatibleProvider` preset instead of duplicating an adapter:

```python
from cubepi.providers.presets import create_provider

provider = create_provider("qwen")  # deepseek, kimi, and glm are also supported
model = provider.model("qwen-plus")
```

Presets use the provider's standard environment variable and endpoint, while
allowing explicit overrides for credentials, `base_url`, headers, request body,
and capabilities. Model feature support remains vendor/model-specific.
