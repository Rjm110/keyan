...
For the built-in OpenAI-compatible presets, use the convenience factory:

```python
from cubepi.providers.presets import create_provider

provider = create_provider("deepseek")  # also: qwen, kimi, glm
model = provider.model("deepseek-chat")
```

The factory reads the matching API key and optional base URL from the
environment. Explicit `api_key`, `base_url`, `capability`, `extra_body`, and
`extra_headers` arguments override the preset. The preset contains defaults,
not a live model or pricing catalog.
