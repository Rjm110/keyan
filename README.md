...
Abstract LLM interaction behind a `Provider` protocol. All providers return `MessageStream` — an async iterator of `StreamEvent`s. Anthropic and OpenAI are built in; DeepSeek, Qwen/Bailian, Kimi, and GLM are available through the shared OpenAI-compatible presets.
