...
OpenAI-compatible presets can be selected explicitly:

```bash
export CUBEPI_PROVIDER=deepseek  # qwen, kimi, or glm
export DEEPSEEK_API_KEY=sk-...
export MODEL=deepseek-chat       # optional
uv run python examples/persistent_chat.py alice
```

The other preset key variables are `DASHSCOPE_API_KEY`, `MOONSHOT_API_KEY`,
and `ZHIPUAI_API_KEY`. Matching `*_BASE_URL` variables override preset
endpoints.
